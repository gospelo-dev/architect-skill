...
| プロパティ | 型     | 必須    | 説明                                   |
| ---------- | ------ | ------- | -------------------------------------- |
| `icon`     | string | **Yes** | アイコン識別子（例: `"aws:lambda"`）   |
| `desc`     | string | No      | リソースの役割の説明                   |

### 7.3 ID 規則

リソース ID は明確にするために `@` プレフィックスで始めるべき（SHOULD）です:
...
- 一致するリソースがないノードは明示的な `icon` プロパティを持たなければなりません（MUST）
...
| プロパティ    | 型               | 必須    | 説明                                                          |
| ------------- | ---------------- | ------- | ------------------------------------------------------------- |
| `id`          | string           | **Yes** | 一意識別子（リソース使用時は `@` で始めるべき（SHOULD））     |
